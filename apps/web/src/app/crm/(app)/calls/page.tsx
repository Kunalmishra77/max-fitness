  ...
  return (
    <>
      ...
      {tasks.length === 0 ? (
        ...
        <ul className="divide-y divide-brand-stone/15 lg:divide-y-0">
          {tasks.map((task) => (
            <li key={task.id} className="bg-white p-4 lg:mb-3 lg:rounded-panel lg:border lg:border-brand-stone/15 lg:shadow-sm">
              ...
              {task.member === null ? null : (
                <div className="mt-3 grid grid-cols-2 gap-3">
                  <a href={`tel:${task.member.mobile}`} className="flex min-h-14 items-center justify-center gap-2 rounded-panel bg-brand-obsidian text-crm-body font-semibold text-white transition-transform hover:-translate-y-0.5">
